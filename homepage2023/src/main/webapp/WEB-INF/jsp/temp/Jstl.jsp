...
<ul>
	<c:choose>
		...
		<c:when test="${step eq '7'}">
			<li>
				<h3>c:param Tag : 포함하는 import, url 태그의 URL의 매개 변수를 추가</h3>
		...
		<c:when test="${step eq '10'}">
			<li>
				...
				<%-- <c:if test="${fn:indexOf(string, "first") > -1}">
				6이라는 단어가 나오면 실행이 될 것이고 -1이면 실행이 안 될 것이다
				-1이 아닌 0이상 부터는 어딘가에 글자가 포함되었다 --%>	
			</li>
		</c:when>
		...
		<c:when test="${step eq '19'}">
			<li>
				<h3>fn:trim() : 문자열의 양쪽 끝에서 공백을 제거(문자열 사이의 공백을 사라지지 않음)</h3>
